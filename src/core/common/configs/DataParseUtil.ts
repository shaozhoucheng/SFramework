module shao {
    ...
    export class DataParseUtil {
        ...
        *
        */
        public static parseDatas(to: Object, from: Object, checkStart: number, checkEnd: number, dataKey: string, typeKey: string, toDatasKey: string): void {
            var arr: any[];
        ...
        */
        public static parseDatas2(to: any, valueList: any[], keyList: string[], checkStart: number, checkEnd: number, dataKey: string, typeKey: string, toDatasKey: string): void {
            let arr: any[];
            for (var i = checkStart; i <= checkEnd; i++) {
                var key: string = dataKey + i;
         ...
         * @param keyList   属性列表
         */
        public static getData(valueList: any[], keyList: string[]): Object {
            let o = {};
         ...
         * @param keyList   属性列表
         */
        public static getDataList(dataList: any[][], keyList: string[]): Object[] {
            let list = [];
            if (dataList) {
         ...
         * @param args
         */
        public static parseDataList(dataList: any[][], keyList: string[], forEach: { (t: Object, args: any[], idx?: number) }, thisObj: any, ...args) {
            if (dataList) {
                for (let i = 0, len = dataList.length; i < len; i++) {
                    let valueList = dataList[i];
         ...
         * @param keyList       属性列表
         */
        public static copyData<T>(to: T, valueList: any[], keyList: string[]) {
            for (let i = 0, len = keyList.length; i < len; i++) {
                let key = keyList[i];
                to[key] = valueList[i];
            }
         ...
         * @param args
         */
        public static copyDataList<T>(creator: { new (): T }, dataList: any[][], keyList: string[], forEach: { (t: T, args: any[], idx?: number) }, thisObj: any, ...args) {
            if (dataList) {
                for (let i = 0, len = dataList.length; i < len; i++) {
                    let valueList = dataList[i];
                    let to = new creator();
         ...
         */
        public static copyDataListForCfg<T extends ICfg>(creator: { new (): T }, dataList: any[][], forEach?: { (t: T, args?: any[], idx?: number): boolean }, thisObj?: any, args?: any[]) {
            if (dataList) {
                for (let i = 0, len = dataList.length; i < len; i++) {
                    let valueList = dataList[i];
                    let to = new creator();
